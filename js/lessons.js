/* ============================================================
   lessons.js: each lesson is a vertical "explorable" with short
   captions + multiple interactive beats. Learn by playing.
   ============================================================ */
import { sha256 } from './sha256.js';
export const LESSONS = (function () {
  "use strict";
  ...
  /* shared rich block/chain renderer, beautiful, detailed, and interactive */
    ...
    function render(freshIdx) {
      ...
      if (opt.trace) {
        ...
        c.querySelectorAll(".xblock").forEach(node => node.onclick = () => {
          ...
          if (i === 0) { node.classList.add("traced"); lm.innerHTML = "This is the <b>genesis</b> block. It is the very first one. It has no previous block, so the chain starts here."; return; }
  ...
  /* merkle tree builder showing each pair of hashes combining up to the root */
  function merkleViz(host, txs, opts) {
    ...
    if (!compact) {
      wrap.querySelector("#mvs").onclick = () => { if (built >= parents.length) return; built++; render(); const node = parents[built - 1]; wrap.querySelector(".mvz-formula").innerHTML = `SHA-256( <b>${short(node.kids[0].hash, 4, 0)}</b> + <b>${short(node.kids[1].hash, 4, 0)}</b> ) = <span class="hl">${short(node.hash, 5, 3)}</span>` + (built >= parents.length ? `<br>That last hash is the <b>Merkle root</b>. It acts as a single fingerprint for all ${txs.length} transactions.` : ""); if (built >= parents.length) wrap.querySelector("#mvs").disabled = true; };
  ...
  function blockLab(host) {
    ...
    function drawBlock() { const root = inB.length ? merk(inB) : null;
      slot.innerHTML = `<div class="xblock${sealed ? " justsealed" : mining ? " mining" : ""}"><div class="xtop"></div><div class="xpad">
        ...
        <div class="xseg"><div class="xlbl">merkle root</div><div class="xv" style="color:var(--gold-text)">${root ? short(root, 11, 7) : "(none)"}</div></div>
  ...
  L.ledger = { world: "primer", title: "The ledger", oneliner: "How a ledger tracks value", icon: "₿",
    hero: "Money is a record of who owns what. Coins and notes are just physical tokens for updating that record, and this course starts with the record itself.",
    beats: [
      { n: "01", h: "Money is just a list", cap: "Press send and watch the entries change. The coins never move. Only the <b>record</b> of who owns them changes.",
      ...
      { n: "02", h: "So who keeps the list?", cap: "A bank keeps one master copy. A blockchain gives <b>everyone</b> a copy. Try cheating both and see which one shrugs it off.",
        build(s) { let mode = "d"; const wrap = el("div", "fcard");
          ...
          function render() { setB();
            if (mode === "c") { body.innerHTML = `<p class="note" style="margin-bottom:10px">One server holds the only copy.</p><div class="kvs" id="t"></div><div class="btn-row" style="margin-top:12px"><button class="btn danger" id="h">Hack the bank</button><button class="btn" id="r">Reset</button></div><div id="m" style="margin-top:12px"></div>`;
              ...
              body.querySelector("#h").onclick = () => { bal.You = 9999; d(); body.querySelector("#m").innerHTML = `<div class="sig-state bad">The single ledger was rewritten. No second copy exists to disagree. Everyone must accept it.</div>`; };
              ...
              body.querySelector("#h").onclick = () => { nodes[2] = 9999; d(); body.querySelector("#m").innerHTML = `<div class="sig-state ok">Node 3 lies, but the other four copies still agree with each other. The odd one out is ignored. No single node can rewrite everyone's copy.</div>`; };
    ...
    deeper: P("Removing the trusted keeper is what defines a blockchain, but it creates a hard new problem. With no boss, how does everyone agree on the one true ledger, and in what order? That question is <b>consensus</b>, and it shapes the rest of this course. The bank's strength (one authority) is also its weakness (one point of control and failure).") },
  ...
  L.doublespend = { world: "primer", title: "Double-spend", oneliner: "Solving the digital double-spend problem", icon: "⊘",
    ...
    beats: [
      { n: "01", h: "A coin is a file, and files copy", cap: "Click to copy the same coin to two people. Nothing physical stops you, only a rule the network agrees on can.",
        build(s) { const wrap = el("div", "fcard"); wrap.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;gap:26px;padding:10px 0">
            ...
            <div style="display:flex;flex-direction:column;gap:10px"><div class="bfield" id="dB" style="min-width:120px">Bob <span class="note" style="float:right" id="dBs">(none)</span></div><div class="bfield" id="dC" style="min-width:120px">Carol <span class="note" style="float:right" id="dCs">(none)</span></div></div></div>
            ...
            <div class="note" id="dMsg" style="text-align:center;margin-top:12px">Both copies look identical. There's no way to tell which one is "real."</div>`;
          s.appendChild(wrap);
          ...
          wrap.querySelector("#dRst").onclick = () => { wrap.querySelector("#dBs").textContent = "(none)"; wrap.querySelector("#dCs").textContent = "(none)"; wrap.querySelector("#dB").style.borderColor = ""; wrap.querySelector("#dC").style.borderColor = ""; wrap.querySelector("#dMsg").innerHTML = "Both copies look identical. There's no way to tell which one is \"real.\""; };
        } },
      { n: "02", h: "The fix needs agreement on order", cap: "Pay Bob, then pay Carol the same coin. Both signatures are valid. Only an agreed <b>order</b> can break the tie.",
        build(s) { const wrap = el("div", "fcard"); wrap.innerHTML = `<div class="btn-row" style="justify-content:center;margin-bottom:12px"><button class="btn primary" id="d1">Pay Bob</button><button class="btn danger" id="d2" disabled>Pay Carol (same coin)</button></div><div class="log" id="dL"><div class="info">network sees:</div></div>`;
          ...
          wrap.querySelector("#d2").onclick = (e) => { e.target.disabled = true; log("tx2 · You → Carol · <span style='color:var(--green)'>also valid</span>"); log("both signatures check out. Signatures prove who authorised each payment; they say nothing about which one landed first", "warn"); log("an agreed order across the whole network is what resolves this → consensus", "info"); };
        } },
    ],
    deeper: P("The problem splits in two. <b>Authorisation</b> (proving you own the coin) is solved by a signature. <b>Ordering</b> (agreeing which payment came first) needs everyone to settle on one shared order of transactions. A plain vote won't do it, because identity online is free: one person can spin up a million fake nodes (a <b>Sybil attack</b>). So a vote must cost something real, like electricity (Proof of Work) or staked capital (Proof of Stake). The network is secure because lying is expensive.") + "<blockquote>“Digital signatures provide part of the solution, but the main benefits are lost if a trusted third party is still required to prevent double-spending. We propose a solution to the double-spending problem using a peer-to-peer network.” (Satoshi Nakamoto, the Bitcoin whitepaper, 2008)</blockquote>" + P("Signatures already proved ownership before Bitcoin existed. What the new machine adds is a way for strangers to agree on the order those signed payments happened in.") };
  ...
  L.hashing = { world: "crypto", title: "Hashing", oneliner: "Digital fingerprints that cannot be faked", icon: "#",
    ...
    beats: [
      { n: "01", h: "The hashing machine", cap: "Type anything. SHA-256 turns it into exactly <b>64 characters</b>. It is the same length whether you type a word or a novel.",
      ...
      { n: "02", h: "Change one letter (half the bits flip)", cap: "Edit input B by one character and watch what happens: close to half of the 256 bits flip, scattered with no pattern. That's the <b>avalanche effect</b>.",
      ...
      { n: "03", h: "And there's no way back", cap: "Here's a hash of a secret word. There is no way to <b>compute</b> backwards. Guessing is the only attack. Hashing throws information away.",
        build(s) { const SECRET = "marigold"; const target = sha256(SECRET); const wrap = el("div", "fcard");
          wrap.innerHTML = `<div class="bfield full"><div class="k">a hash of some secret word</div><div class="v go">${short(target, 20, 10)}</div></div><div style="margin-top:12px"><input class="in" id="gIn" placeholder="try to guess the word…"></div><div class="sig-state" id="gOut" style="margin-top:10px">Type a guess.</div><div class="note" style="margin-top:8px">There's no formula, only guessing. Hopeless for a random 256-bit secret, very possible for a guessable word. That's exactly how leaked password hashes get cracked.</div>`;
          s.appendChild(wrap); const inp = wrap.querySelector("#gIn"), out = wrap.querySelector("#gOut"); let tries = 0;
          inp.oninput = () => { if (!inp.value) { out.textContent = "Type a guess."; out.className = "sig-state"; return; } tries++; const h = sha256(inp.value); if (h === target) { out.innerHTML = "You found it because it's a guessable word. Guessing wins against weak passwords all the time. Against a random 256-bit secret, it doesn't stand a chance."; out.className = "sig-state ok"; } else { out.innerHTML = `<span class="mono" style="font-size:11px">${short(h, 14, 8)}</span> (no match, ${tries} tries)`; out.className = "sig-state bad"; } };
        } },
    ],
    deeper: P("SHA-256 returns <b>256 bits</b> (64 hex characters). Infinitely many inputs map to only 2²⁵⁶ outputs, and the function <b>destroys information</b> on the way, so there's no running it backwards. Reversing a specific output would take longer than the age of the universe. Security rests on three guarantees: <b>preimage</b> resistance (can't find an input for an output), <b>second-preimage</b> resistance (can't find another input matching one), and <b>collision</b> resistance (can't find any two inputs that match). Hashing differs from encryption in one basic way: encryption is reversible with the right key, hashing throws the input away for good. The birthday paradox cuts collision search to about 2¹²⁸ work instead of 2²⁵⁶, still astronomical, which is why SHA-256 holds today while SHA-1 (2⁸⁰) is broken.") };

  L.keys = { world: "crypto", title: "Keys & signatures", oneliner: "Proving ownership using digital signatures", icon: "⚿",
    hero: "There are no usernames or passwords here. A secret number is the only thing that proves something is yours.",
    beats: [
      { n: "01", h: "One secret, flowing one way", cap: "A private key makes a public key; the public key makes an address. The chain only runs <b>one direction</b>: forward.",
    ...
    deeper: P("This is <b>ECDSA</b>, the same signature scheme Bitcoin and Ethereum use, on a sibling curve (they use secp256k1 while this demo uses P-256). The trapdoor works like this: your public key is a fixed point added to itself a secret number of times. Going forward is fast. Going back (the discrete-log problem) is hopeless. Signing covers the <b>hash</b> of the transaction, proving authenticity and integrity at once. The verifier learns only that this key signed this message, nothing about your secret. Lose the key and the coins freeze forever. Copy the key and whoever holds it can spend exactly as you could, with nothing to tell them apart from you.") };

  /* ===================== BUILDING THE CHAIN ===================== */
  L.block = { world: "chain", title: "A block", oneliner: "Bundling and sealing network data", icon: "▦",
    hero: "A block is a box. Drag some transactions in and then mine it. Watch the fingerprint churn through thousands of guesses before it locks shut.",
    beats: [
    ...
    deeper: P("Each transaction is hashed; pairs of hashes are combined and hashed again, up a tree, until one <b>Merkle root</b> remains in the header. Hashing the whole header gives the block its ID, or its seal. A block on its own isn't special. Anyone can build one. What makes <i>adding</i> it costly, and the past unchangeable, is the lessons ahead: the nonce, the reward, and the chain.") };

  L.nonce = { world: "chain", title: "The nonce", oneliner: "Solving the Proof of Work puzzle", icon: "⛏",
    hero: "A finished block is frozen. The transactions, the link, the time are all set. A miner can change exactly one thing: a throwaway number called the nonce (no relation to the transaction counter that borrows the same name). Mining is just turning that one dial.",
    beats: [
      { n: "01", h: "Everything is locked but one number", cap: "Look at the block below. The data, the link to the last block, the merkle root, the time are all <b>fixed</b>. The miner is only allowed to change the <span class='k'>nonce</span>. Turn the dial and watch. Only it and the hash move. Nothing else can.",
    ...
    deeper: P("If <code>p = target / 2²⁵⁶</code> is the chance one hash qualifies, expected tries is <code>1/p</code>, and each extra zero of difficulty makes it 16× rarer (a hex digit has 16 values). Bitcoin runs about 10²³ hashes per block every ten minutes, re-tuning the target every two weeks to hold that pace. The winner mints new coins plus fees. Producing a block costs a staggering number of guesses; checking one is a single hash. That gap is why mining can secure the network without anyone having to trust the miner.") };

  L.chainlink = { world: "chain", title: "The chain", oneliner: "Cryptographically locking history in place", icon: "⛓",
    hero: "Line the blocks up. Each one writes down the fingerprint of the one before it, so the blocks are locked together by their own hashes.",
    beats: [
      { n: "01", h: "Follow the link between two blocks", cap: "A block's <b>‘prev’</b> field holds the <b>exact fingerprint</b> of the block before it. Click any block to light up the link and see the two matching values for yourself.",
      ...
      { n: "02", h: "Watch new links form", cap: "Every new block grabs the current tip's fingerprint, writes it into its own ‘prev’ field, and then gets mined to seal it. Mine a few and watch each new block lock onto the one before it.",
    ...
    deeper: P("Changing the past edits far more than one block: it invalidates everything built on top, in plain view. To repair it you'd have to re-mine that block <i>and every block after it</i>, winning the whole Proof-of-Work race again, while the honest network keeps extending the real chain with all its power. Below 50% of the hashrate, you fall further behind every ten minutes. Fingerprints make tampering visible, and Proof of Work makes fixing it a race the tamperer loses. That combination is what immutability actually means.") };
  ...
  L.merkle = { world: "chain", title: "Merkle tree", oneliner: "Packing thousands of transactions efficiently", icon: "⋔",
    ...
    deeper: P("The tree's height is <code>log₂(n)</code>, so a million transactions need ~20 sibling hashes, and a billion need ~30. Doubling the block adds just one hash to the proof. A fake sibling produces the wrong root, so the proof fails safely even from an untrusted source. This is what lets a phone wallet confirm a payment in milliseconds without ever downloading the chain.") };

  /* ===================== CONSENSUS ===================== */
  L.forks = { world: "consensus", title: "Forks", oneliner: "How the decentralised network agrees", icon: "⑂",
    hero: "Two miners can win at the same instant, with no referee to decide which block is real.",
    beats: [
      ...
      { n: "02", h: "Why you wait for confirmations", cap: "Each new block stacked on top of yours makes it exponentially harder to undo. Stack a few and watch the chance of reversal fall off a cliff.",
        build(s) { let conf = 0; const wrap = el("div", "fcard"); wrap.innerHTML = `<div class="flabel"><span class="pin"></span>burying a payment</div><div id="stack" style="display:flex;flex-direction:column-reverse;gap:5px;align-items:center;min-height:50px"></div><div style="text-align:center;margin:12px 0"><div class="note">chance a determined attacker with 10% of the mining power could still reverse it (the whitepaper's own formula)</div><div class="mono" id="risk" style="font-size:30px;font-weight:700"></div></div><div class="btn-row" style="justify-content:center"><button class="btn primary" id="add">+ One confirmation</button><button class="btn" id="reset">Reset</button></div>`;
          s.appendChild(wrap);
    ...
    deeper: P("A recent block is never fully final because a deeper competing branch could replace it (a <b>reorg</b>). You get <b>probabilistic finality</b> instead: every block stacked on top makes reversal exponentially harder. ‘Wait for six confirmations’ (about an hour) is exactly this idea with a number on it. Confidence builds with each confirmation rather than arriving at some single certain moment.") };
  ...
  L.attack = { world: "consensus", title: "The 51% attack", oneliner: "The anatomy of a 51% attack", icon: "½",
    ...
    beats: [
      { n: "01", h: "A race you usually lose", cap: "You're trying to reverse a payment by secretly out-mining the network. Set your share and the confirmations waited. The odds are the whitepaper's own formula, computed live.",
      ...
      { n: "02", h: "Even a majority has limits", cap: "Controlling most of the mining is powerful, but the cryptography still holds. Here is exactly where the line sits.",
        build(s) { const can = ["Reverse its own recent payments", "Block (censor) transactions", "Out-race the honest chain"]; const cant = ["Forge anyone else's signature", "Steal coins it has no key for", "Mint coins out of nothing", "Change the protocol's rules"];
    ...
    deeper: P("With a majority you <b>can</b> double-spend your own recent payments and censor transactions; you <b>cannot</b> forge signatures, steal coins you have no key for, or change the rules. Below 50%, catch-up probability decays exponentially with each confirmation (Satoshi's gambler's-ruin result); at 50% it becomes certain, a sharp jump rather than a gradual slide. Subtler threats: <b>selfish mining</b> pays off above ~⅓ of hashrate, and <b>pools</b> can drift toward majority over time. Security here is economic, not absolute. Attacks are deterred by costing more than they're worth.") + "<blockquote>“With the odds against him, if he doesn't make a lucky lunge forward early on, his chances become vanishingly small as he falls further behind.” (Satoshi Nakamoto, the Bitcoin whitepaper, 2008, §11)</blockquote>" + P("The demo above runs that section's own calculation live. The paper is just as clear about the ceiling on the damage: even a successful attacker “can only try to change one of his own transactions to take back money he recently spent”. Every node still rejects forged signatures and conjured coins, no matter whose chain is longer.") };
  ...
  L.pos = { world: "consensus", title: "Proof of Stake", oneliner: "Securing networks by staking capital", icon: "◈",
    ...
    beats: [
      { n: "01", h: "Stake, propose, or get slashed", cap: "Validators win blocks in proportion to their stake. Make one cheat and the protocol <b>burns a slice of its bond</b> and ejects it. This is called slashing. The more validators cheat together, the bigger the slice, up to everything.",
        build(s) { let vals = [{ n: "A", stake: 32, c: "#620d3c", w: 0, x: false }, { n: "B", stake: 96, c: "#f1a222", w: 0, x: false }, { n: "C", stake: 64, c: "#8a2057", w: 0, x: false }, { n: "D", stake: 32, c: "#2e9e6b", w: 0, x: false }];
          const wrap = el("div", "fcard"); wrap.innerHTML = `<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(100px,1fr));gap:10px" id="pG"></div><div class="btn-row" style="margin-top:16px;justify-content:center"><button class="btn primary" id="pR">Propose 50 blocks</button><button class="btn danger" id="pC">Make B double-sign</button><button class="btn" id="pRs">Reset</button></div><div class="log" id="pL" style="margin-top:14px"><div class="info">beacon chain</div></div>`;
          ...
          wrap.querySelector("#pC").onclick = () => { const b = vals[1]; if (b.x) return; log("Val B signs two conflicting blocks at one height", "warn"); const burned = Math.round(b.stake / 32); b.x = true; b.stake -= burned; grid(); log(`Val B slashed. ${burned}Ξ burned (the initial penalty) and ejected from the validator set.`, "bad"); log("If many validators cheated together, a correlation penalty would scale the burn toward the entire stake.", "info"); };
      ...
      { n: "02", h: "And it sips energy", cap: "Same security, almost none of the power. Switching from mining to staking cut Ethereum's energy use by about 99.95%.",
    ...
    deeper: P("Faking a thousand validators means risking a thousand times the money. It's the same Sybil defence as mining, but the scarce resource is capital rather than energy. Ethereum's 2022 switch cut its energy use ~99.95%. The debate is genuinely open. PoW's cost is external and physical (you can't fake electricity). PoS's attacker has their own stake slashed (the attack destroys their capital). The shared worry is that wealth and staking pools concentrate control.") + P("On Ethereum the punishment comes in instalments. A slashed validator loses a small slice of its stake at once and is queued for ejection, about 36 days on its way out. Halfway through comes the <b>correlation penalty</b>. It scales with how much <i>other</i> stake was slashed around the same time. A lone fault costs little, while a coordinated attack can burn <a href='https://ethereum.org/en/developers/docs/consensus-mechanisms/pos/rewards-and-penalties/' target='_blank' rel='noopener'>the entire stake</a>. What actually burns is the staked coins themselves. A validator caught cheating loses real capital, permanently, on top of any rewards it forfeits.") };
  ...
  <span style="color:#79e0c0">function</span> buy() payable {
    ...
    stock--; emit Dispensed(msg.sender);
      ...
      { n: "02", h: "Immutable code, immutable bugs", cap: "Because a deployed contract cannot be patched, a single flaw is a vault with a hole. Trigger the bug and watch the funds drain. There is no admin to stop it.",
        build(s) { let bal = 1000, t = null; const wrap = el("div", "fcard"); wrap.innerHTML = `<div class="flabel"><span class="pin"></span>a buggy vault contract</div><div style="text-align:center"><div class="metric"><span class="n" id="v" style="color:var(--green)">1,000</span> <span class="u">coins locked in the contract</span></div></div><div class="btn-row" style="justify-content:center;margin-top:14px"><button class="btn danger" id="hack">Exploit the bug</button><button class="btn" id="reset">Reset</button></div><div class="sig-state" id="msg" style="margin-top:12px">The code runs exactly as written, flaws included.</div>`;
          s.appendChild(wrap); wrap.querySelector("#hack").onclick = () => { if (t) return; t = setInterval(() => { bal = Math.max(0, bal - Math.ceil(bal * 0.25)); wrap.querySelector("#v").textContent = bal.toLocaleString(); wrap.querySelector("#v").style.color = "var(--red)"; if (bal === 0) { clearInterval(t); t = null; wrap.querySelector("#msg").className = "sig-state bad"; wrap.querySelector("#msg").textContent = "Drained, with no admin to pause it and no way to undo it. Billions have been lost exactly like this."; } }, 200); };
    ...
    deeper: P("A smart contract is just a deterministic program that every node runs in lockstep, agreeing on the same result. <b>Gas</b> charges for each operation, rationing the shared computer and stopping infinite loops. This powers <b>DeFi</b>, <b>NFTs</b>, and <b>DAOs</b>, which are all just contracts moving tokens by rules. The catch is that immutable code means immutable <b>bugs</b>, and the funds sit inside the contract. (Unpatchable <i>by default</i>, that is. Many teams deploy behind an upgradeable proxy, which fixes the bug problem by reintroducing a trusted party.) ‘Code is law’ collides with the law's need to reverse fraud. Billions have been lost to contract exploits.") };

  L.zk = { world: "frontier", title: "Zero-knowledge", oneliner: "Proving facts without revealing secrets", icon: "◇",
    hero: "Prove you know a secret without revealing the secret. This is the frontier of privacy and scaling.",
    beats: [
      { n: "01", h: "The cave game", cap: "Peggy claims she knows a secret word. Victor wants proof. Run rounds as an honest prover, then as a cheater. Watch a bluffer's luck run out, while the word is never spoken.",
      ...
      { n: "02", h: "Why it matters", cap: "That one trick, proving something is true while revealing nothing else, unlocks three things in practice.",
        build(s) { const items = [["🔒 Private payments", "Send money on a public chain while hiding the amount and the parties (Zcash)."], ["📦 zk-rollups", "Bundle thousands of transactions into one tiny proof, scaling the chain cheaply."], ["✅ Compliance without exposure", "Prove you are not on a sanctions list without revealing who you are."]];
    ...
    deeper: P("Real <b>zk-SNARKs / zk-STARKs</b> compress this into a tiny proof that ‘I ran this computation correctly’, cheap to check even when the computation was huge. (In rollups the trick is used mostly for <i>compactness</i>, not secrecy, because many zk-rollups publish all their transaction data.) That powers private payments and <b>zk-rollups</b>, which bundle thousands of transactions off-chain and post one proof to settle them, scaling the chain without trusting anyone. Privacy-preserving compliance is the policy frontier: prove you're not on a sanctions list without revealing your identity.") };
  ...
  L.money = { world: "frontier", title: "Money & the state", oneliner: "Understanding stablecoins and CBDCs", icon: "$",
    ...
    beats: [
      { n: "01", h: "Slide from freedom to control", cap: "Drag from permissionless crypto to a <b>CBDC</b>, a digital currency issued by the central bank itself. Same digital money, pointed in opposite directions.",
    ...
    deeper: P("<b>Stablecoins</b> hold a peg. They can be fiat-backed (a dollar in reserve), crypto-backed (over-collateralised), or algorithmic (held by code, and fragile since Terra collapsed). A <b>CBDC</b> is a central bank's own digital currency, and technically the opposite of crypto: centralised, permissioned, fully controlled. Take the referee out of the system and you get money no state can freeze. Hand the state the keys instead and you get the most controllable money ever built. Either way, the policy question comes down to who holds the keys.") };

  L.tokens = { world: "frontier", title: "Tokens & NFTs", oneliner: "Creating digital assets and NFTs", icon: "◎",
    hero: "Once a chain can run code, it can track more than coins. A token is just an entry in a contract's ledger. An NFT is a token that is one of a kind.",
    beats: [
      { n: "01", h: "Mint and move tokens", cap: "A token is just a row in a contract saying how many you hold. Mint some and send them around, and the contract keeps the count.",
  ...
  L.wallets = { world: "frontier", title: "Wallets & custody", oneliner: "Holding and protecting your own keys", icon: "❖",
    ...
    deeper: P("This is the trade-off behind ‘not your keys, not your coins’. Self-custody gives you sovereignty and full responsibility; custodial services give you convenience and a recovery path, at the cost of trusting them, the very intermediary blockchains were built to remove. Most regulation targets these custodians, because they are the one place a government can actually apply pressure.") + P("The demo's phrase mirrors a real standard: <b>BIP-39</b>, which turns 128–256 bits of randomness into 12–24 words drawn from a fixed <a href='https://github.com/bitcoin/bips/blob/master/bip-0039.mediawiki' target='_blank' rel='noopener'>2,048-word list</a>. The phrase is stretched into a seed, and every key and address you will ever use is derived from that seed, deterministically. This is why typing the same words into any brand of wallet resurrects everything, and why anyone who so much as reads them owns all they derive.") };

  L.layer2 = { world: "frontier", title: "Layer 2 & scaling", oneliner: "Scaling the network for global use", icon: "⇈",
    hero: "A secure, decentralised chain is slow on purpose, because every computer re-checks every transaction. Layer 2 buys back the speed without giving that up.",
    beats: [
    ...
    deeper: P("Two main flavours. <b>Optimistic rollups</b> assume each batch is valid and allow a challenge window to prove fraud. <b>zk-rollups</b> attach a cryptographic proof that the batch is valid (the zero-knowledge maths you'll meet in the next lesson), so there is nothing to dispute. Either way the heavy work happens off-chain and only a tiny summary touches the expensive, secure base layer, buying scale without selling decentralisation or security.") + P("The security really is inherited from the base chain. A rollup must post its compressed transaction data there. <a href='https://ethereum.org/en/developers/docs/scaling/zk-rollups/' target='_blank' rel='noopener'>ethereum.org's docs</a> note that zk-rollups “write transactions to Ethereum as calldata”, so anyone can reconstruct the rollup's state from Layer 1 alone, and no operator can trap funds by withholding data. You pay Layer 1 prices for a few compressed bytes per transaction, well below the cost of re-executing it there.") };

  /* ===================== PRIMER (start here) ===================== */
  L.whatis = { world: "foundations", title: "What is a blockchain?", oneliner: "The entire concept in one screen", icon: "◧",
    hero: "Forget the buzzwords. A blockchain is a list of blocks, copied across many computers, that nobody can secretly rewrite. Let us build that picture.",
    beats: [
      { n: "01", h: "A block holds records", cap: "Start with one <b>block</b>. It is just a box that holds a list of records, like a page in a notebook. Add a few and watch its fingerprint at the bottom change.",
      ...
      { n: "03", h: "And everyone keeps a copy", cap: "There is no master copy. The exact same chain lives on thousands of computers. Try to tamper with one and its copy stops matching everyone else's, so the network simply <b>ignores</b> it.",
        build(s) { const wrap = el("div", "fcard"); wrap.innerHTML = `<div class="flabel"><span class="pin"></span>the same chain, everywhere</div><div class="copies" id="cps"></div><div class="btn-row" style="justify-content:center;margin-top:14px"><button class="btn danger" id="tamper">Tamper with one copy</button><button class="btn" id="reset">Reset</button></div><div class="note" id="msg" style="text-align:center;margin-top:12px">All copies agree on the same history.</div>`;
          ...
          wrap.querySelector("#tamper").onclick = () => { bad = Math.floor(Math.random() * N); draw(); wrap.querySelector("#msg").innerHTML = `<span style="color:var(--red)">Computer ${bad + 1} now disagrees with the other ${N - 1}. Their copies all match each other, so the odd one out is ignored. (This isn't literally a vote: faking an identity online is free, so the network has to earn agreement some other way, which the consensus lessons cover.)</span>`; };
          wrap.querySelector("#reset").onclick = () => { bad = -1; draw(); wrap.querySelector("#msg").textContent = "All copies agree on the same history."; }; draw();
        } },
    ],
    deeper: P("That is the entire mental model: <b>blocks</b> of records, <b>chained</b> by fingerprints, <b>copied</b> across a whole network. The clever part is how the three combine: together they let strangers keep one shared, tamper-proof record without trusting anyone in charge. Everything else in this course is just <i>how</i> each of those three words actually works.") };
  ...
  L.why = { world: "primer", title: "Why do we need it?", oneliner: "Why we actually need a blockchain", icon: "?",
    ...
    beats: [
      ...
      { n: "02", h: "But one keeper is one weak point", cap: "Everything sits with a single party. Press the buttons to see a frozen account or a failed server, and your money is suddenly out of reach.",
      ...
      { n: "03", h: "Blockchain removes the keeper", cap: "Spread the same record across everyone. Now no single party can freeze you, and no single failure can erase it. Try to freeze it and watch nothing happen.",
        build(s) { const wrap = el("div", "fcard"); wrap.innerHTML = `<div class="flabel"><span class="pin"></span>the same balance, on every computer</div><div class="copies" id="cps"></div><div class="btn-row" style="justify-content:center;margin-top:14px"><button class="btn danger" id="freeze">Try to freeze it</button></div><div class="note" id="msg" style="text-align:center;margin-top:12px">Your balance lives on all of them at once.</div>`;
          ...
          wrap.querySelector("#freeze").onclick = () => { wrap.querySelector("#msg").innerHTML = `<span style="color:var(--green)">There is no single keeper to lean on. To freeze you, someone would have to seize a majority of the world's computers at once, which is practically impossible.</span>`; }; draw();
        } },
    ],
    deeper: P("Blockchain trades a <i>trusted</i> keeper for a <i>trustless</i> system. You give up the convenience of someone to call when things go wrong, and in return you get money or records that no single party can censor, freeze, or alter without everyone noticing. Whether that trade is worth it depends entirely on the use case.") };
  ...
  L.tour = { world: "foundations", title: "The life of a payment", oneliner: "The lifecycle of a single payment", icon: "↗",
    ...
    beats: [
      { n: "01", h: "Step through it", cap: "Click through the journey of a single payment. Each step is its own lesson later, so this is just the map.",
        build(s) { const steps = [
            ...
            { ic: "🌍", t: "Every copy updates", d: "Every computer adds the same block. With a few more stacked on top, your payment is final and there are no take-backs." },
          ];
  ...
  L.recap = { world: "capstone", title: "The whole machine", oneliner: "The complete blockchain machine in motion", icon: "★",
    ...
    deeper: P("Every piece in this course supports one shared record that strangers who don't trust each other can agree on, without a referee. Take the referee out and the result is money no state can freeze. Hand a state the keys instead and the result is the most controllable money ever built. Same machine, built to point either way.") };
