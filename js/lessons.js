...
export const LESSONS = (function () {
    ...
    beats: [
      { n: "01", h: "A coin is a file, and files copy", cap: "Click to copy the same coin to two people. Nothing physical stops you. So what does?",
        build(s) { const wrap = el("div", "fcard"); wrap.innerHTML = `<div style="display:flex;align-items:center;justify-content:center;gap:26px;padding:10px 0">
          ...
          wrap.querySelector("#dRst").onclick = () => { wrap.querySelector("#dBs").textContent = "(none)"; wrap.querySelector("#dCs").textContent = "(none)"; wrap.querySelector("#dB").style.borderColor = ""; wrap.querySelector("#dC").style.borderColor = ""; wrap.querySelector("#dMsg").innerHTML = "Both copies look identical. Which is the ‘real’ one?"; };
        } },
    ...
    beats: [
      ...
      { n: "03", h: "And there's no way back", cap: "Here's a hash of a secret word. There is no way to <b>compute</b> backwards. Guessing is the only attack. Hashing throws information away.",
        build(s) { const SECRET = "marigold"; const target = sha256(SECRET); const wrap = el("div", "fcard");
          ...
          inp.oninput = () => { if (!inp.value) { out.textContent = "Type a guess."; out.className = "sig-state"; return; } tries++; const h = sha256(inp.value); if (h === target) { out.innerHTML = "You found it, because it was a guessable word. A random 256-bit secret would never fall this way. Weak passwords do, every day."; out.className = "sig-state ok"; } else { out.innerHTML = `<span class="mono" style="font-size:11px">${short(h, 14, 8)}</span> (no match, ${tries} tries)`; out.className = "sig-state bad"; } };
        } },
    ],
    deeper: P("SHA-256 returns <b>256 bits</b> (64 hex characters). Infinitely many inputs map to only 2²⁵⁶ outputs. It <b>destroys information</b>, so you can't run it backwards. Reversing a specific output would take longer than the age of the universe. Security rests on three guarantees. First is <b>preimage</b> resistance (can't find an input for an output). Second is <b>second-preimage</b> resistance (can't find another input matching one). Third is <b>collision</b> resistance (can't find any two that match). The birthday paradox means collisions take ~2¹²⁸ work instead of 2²⁵⁶. That is still astronomical, which is why SHA-256 holds while SHA-1 (2⁸⁰) is broken. Hashing is not encryption. Encryption is two-way and keeps the data. Hashing is one-way and throws it away.") };
  ...
  L.keys = { world: "crypto", title: "Keys & signatures", oneliner: "Proving ownership using digital signatures", icon: "⚿",
    ...
    beats: [
      { n: "01", h: "One secret, flowing one way", cap: "A private key makes a public key; the public key makes an address. You can walk this chain forwards, <b>never backwards</b>.",
  ...
  L.nonce = { world: "chain", title: "The nonce", oneliner: "Solving the Proof of Work puzzle", icon: "⛏",
    ...
    deeper: P("If <code>p = target / 2²⁵⁶</code> is the chance one hash qualifies, expected tries is <code>1/p</code>, and each extra zero of difficulty makes it 16× rarer (a hex digit has 16 values). Bitcoin runs about 10²³ hashes per block every ten minutes, re-tuning the target every two weeks to hold that pace. The winner mints new coins plus fees. The recurring theme is that producing a block is staggeringly expensive, but <b>checking</b> it is a single hash. That asymmetry is what makes the whole thing work.") };
  ...
  L.layer2 = { world: "frontier", title: "Layer 2 & scaling", oneliner: "Scaling the network for global use", icon: "⇈",
    ...
    deeper: P("Two main flavours. <b>Optimistic rollups</b> assume each batch is valid and allow a challenge window to prove fraud. <b>zk-rollups</b> attach a cryptographic proof that the batch is valid (the zero-knowledge maths you'll meet in the next lesson), so there is nothing to dispute. Either way the heavy work happens off-chain and only a tiny summary touches the expensive, secure base layer, buying scale without selling decentralisation or security.") + P("And the security really is inherited, not promised. A rollup must post its compressed transaction data to the main chain. <a href='https://ethereum.org/en/developers/docs/scaling/zk-rollups/' target='_blank' rel='noopener'>ethereum.org's docs</a> note that zk-rollups “write transactions to Ethereum as calldata”, so anyone can reconstruct the rollup's state from Layer 1 alone, and no operator can trap funds by withholding data. You pay Layer 1 prices for a few compressed bytes per transaction, not for re-executing it.") };
    ...
    beats: [
      ...
      { n: "03", h: "And everyone keeps a copy", cap: "There is no master copy. The exact same chain lives on thousands of computers. Try to tamper with one and its copy stops matching everyone else's, so the network simply <b>ignores</b> it.",
        build(s) { const wrap = el("div", "fcard"); wrap.innerHTML = `<div class="flabel"><span class="pin"></span>the same chain, everywhere</div><div class="copies" id="cps"></div><div class="btn-row" style="justify-content:center;margin-top:14px"><button class="btn danger" id="tamper">Tamper with one copy</button><button class="btn" id="reset">Reset</button></div><div class="note" id="msg" style="text-align:center;margin-top:12px">All copies agree on the same history.</div>`;
          ...
          wrap.querySelector("#tamper").onclick = () => { bad = Math.floor(Math.random() * N); draw(); wrap.querySelector("#msg").innerHTML = `<span style="color:var(--red)">Computer ${bad + 1} now disagrees with the other ${N - 1}. Their copies all match each other, so the odd one out is ignored. (Later you'll see it's not literally a vote: fake identities are free, so agreement has to be earned another way.)</span>`; };
