...
window.LESSONS = (function () {
  ...
  L.keys = { world: "crypto", title: "Keys & signatures", oneliner: "Proving ownership using digital signatures", icon: "⚿",
    ...
    deeper: P("This is <b>ECDSA</b> — the same signature scheme Bitcoin and Ethereum use, on a sibling curve (they use secp256k1; this demo uses P-256). The trapdoor: your public key is a fixed point added to itself a secret number of times; going forward is fast, going back (the discrete-log problem) is hopeless. Signing covers the <b>hash</b> of the transaction, so it proves authenticity and integrity at once — and the verifier learns only ‘this key signed this’, never your secret. Lose the key and the coins freeze forever; copy it and the thief simply is you.") };
  ...
  L.nonce = { world: "chain", title: "The nonce", oneliner: "Solving the Proof of Work puzzle", icon: "⛏",
    hero: "A finished block is frozen — the transactions, the link, the time. A miner can change exactly one thing: a throwaway number called the nonce (no relation to the transaction counter that borrows the same name). Mining is just turning that one dial.",
    beats: [
  ...
  L.pos = { world: "consensus", title: "Proof of Stake", oneliner: "Securing networks by staking capital", icon: "◈",
    ...
    beats: [
      { n: "01", h: "Stake, propose, or get slashed", cap: "Validators win blocks in proportion to their stake. Make one cheat and the protocol <b>burns a slice of its bond</b> and ejects it — slashing. The more validators cheat together, the bigger the slice, up to everything.",
  ...
  <span style="color:#79e0c0">function</span> buy() payable {
    ...
    deeper: P("It isn't ‘smart’ and isn't a ‘contract’: it's a deterministic program every node runs in lockstep, agreeing on the result. <b>Gas</b> charges for each operation, rationing the shared computer and stopping infinite loops. This powers <b>DeFi</b>, <b>NFTs</b>, and <b>DAOs</b> — all just contracts moving tokens by rules. The catch: immutable code means immutable <b>bugs</b>, and the funds sit inside the contract. (Unpatchable <i>by default</i>, that is — many teams deploy behind an upgradeable proxy, which fixes the bug problem by reintroducing a trusted party.) ‘Code is law’ collides with the law's need to reverse fraud — billions have been lost to contract exploits.") };
  ...
  L.whatis = { world: "foundations", title: "What is a blockchain?", oneliner: "The entire concept in one screen", icon: "◧",
    ...
    beats: [
      ...
      { n: "03", h: "And everyone keeps a copy", cap: "There is no master copy. The exact same chain lives on thousands of computers. Try to tamper with one — its copy stops matching everyone else's, and the network simply <b>ignores</b> it.",
